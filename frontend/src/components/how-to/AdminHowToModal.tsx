...
const STEPS: Step[] = [
    {
        title: "The Process Architect",
        description: "Think of your business as a high-performance assembly line. As an admin, you aren't just managing tasks—you are building the digital conveyor belt that makes chaos impossible.",
        color: "blue",
    },
    {
        title: "1. Define the Blueprints",
        description: "Create standardized Templates. These are the flawless blueprints that dictate exactly how a service is delivered every single time, eliminating guesswork.",
        color: "indigo",
    },
    {
        title: "2. Assign the Operators",
        description: "Map your team to specific roles. When a workflow begins, the system automatically routes the right task to the right expert without a single email.",
        color: "purple",
    },
    {
        title: "3. Establish Checkpoints",
        description: "Set up mandatory Quality Gates and Approvals. Work cannot move down the line until it meets your exact standards, ensuring premium delivery.",
        color: "emerald",
    },
    {
        title: "4. Monitor the Belt",
        description: "Watch your business breathe. Use real-time dashboards to identify bottlenecks instantly and ensure no task is ever left behind.",
        color: "amber",
    },
    {
        title: "Total Optimization",
        description: "By architecting your operations this way, you remove friction entirely. Your team focuses on execution, and your business scales flawlessly.",
        color: "emerald",
    }
];
...
interface AdminHowToModalProps {
    userName?: string;
    open: boolean;
    onOpenChange: (open: boolean) => void;
}

export function AdminHowToModal({ userName, open, onOpenChange }: AdminHowToModalProps) {
    ...
    const renderRightPanel = () => {
        if (currentStep === 5) {
            ...
            return (
                <div className="w-full h-full flex flex-col items-center justify-center p-6 md:p-8 bg-[#0a0a0f] text-white rounded-2xl md:rounded-3xl relative overflow-hidden">
                    ...
                    >
                        <div className="w-20 h-20 md:w-32 md:h-32 rounded-full bg-emerald-500/20 flex items-center justify-center border-4 border-emerald-500/50 mb-6 md:mb-8 shadow-[0_0_50px_rgba(16,185,129,0.4)]">
                            <TrendingUp className="w-10 h-10 md:w-16 md:h-16 text-emerald-400" />
                        </div>
                        <h3 className="text-3xl md:text-4xl font-black text-emerald-400 tracking-tight mb-2">10/10</h3>
                        <p className="text-lg md:text-xl font-bold text-white mb-6 md:mb-8">System Optimized</p>

                        <div className="grid grid-cols-2 gap-3 md:gap-4 w-full max-w-sm">
                            {[
                                { icon: Clock, label: "Efficiency", value: "Maximized" },
                                { icon: Shield, label: "Compliance", value: "Automated" },
                                { icon: Users, label: "Delegation", value: "Seamless" },
                                { icon: BarChart3, label: "Oversight", value: "Absolute" },
                            ].map((stat, i) => (
                                <motion.div 
                                    key={i}
                                    ...
                                    className="bg-white/5 border border-white/10 rounded-xl p-3 md:p-4 flex flex-col items-center justify-center"
                                >
                                    <stat.icon className="w-5 h-5 md:w-6 md:h-6 text-emerald-400 mb-1 md:mb-2" />
                                    <span className="text-[10px] md:text-xs text-gray-400">{stat.label}</span>
        ...
        return (
            <div className="w-full h-full flex flex-col justify-center px-6 md:px-12 relative bg-white">
                <div className="absolute left-10 md:left-[3.25rem] top-1/4 bottom-1/4 w-0.5 bg-gray-100 z-0 hidden md:block" />
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="w-[95vw] sm:max-w-6xl p-0 overflow-y-auto sm:overflow-hidden border-none shadow-2xl bg-white/95 backdrop-blur-xl rounded-2xl sm:rounded-3xl max-h-[95vh] sm:h-auto">
                <Button
                    variant="ghost"
                    size="icon"
